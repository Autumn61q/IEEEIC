...
y = 5 * x + 0.8 * torch.rand(x.size())  # .rand()生成0到1之间的随机数
...
# 定义参数初始化函数
def init_parameters():
    W = Variable( torch.randn(1, 1), requires_grad=True)  # 随机初始化 w  # 生成的是1x1的矩阵
    b = Variable( torch.zeros(1, 1), requires_grad=True )  # 初始化偏差   #  注意bias均为零。合理
    parameters = {"W": W, "b": b}
    return parameters

# 定义模型
def model(X, parameters):
    return X * parameters["W"] + parameters["b"]  # 想一下李宏毅老师讲的用不同日期来预测播放量的例子

# 定义损失函数
def square_loss(y_hat, Y):
    loss = (y_hat - Y).pow(2).sum()  # .pow(2)和.sum()来自torch.Tensor()
    return loss
...
# 需要传入的参数是: in_features, out_features 也就是输出特征和输出特征  # 其实就是输入的变量的个数，和输出的神经元的个数
