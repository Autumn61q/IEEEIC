...
# 创建一个随机的二维数组（矩阵）
exam1 = torch.randn(2, 3)  #.randn()这个函数生成的tensor的数据是按照均值为0，标准差为1分布的
print(exam1)
...
shape = exam4.size()
print(type(shape))  # print(shape) : torch.Size([2, 3]) 
print(shape[0]) # print 2
print(shape[1])
...
print(x_sq)
x_sq_1 = torch.squeeze(x_u, 0) # 后面第二个参数是要操作的维度。操作维度若为1，则删除该维度，若不为1，则不变。可见106行的例子
print(x_sq_1)

test1 = torch.Tensor([[1], [2], [3]])  # 别忘了写两层括号
print(test1)
# tensor([[1.],
#         [2.],
#         [3.]])
print(torch.squeeze(test1, 1))
# tensor([1., 2., 3.])
